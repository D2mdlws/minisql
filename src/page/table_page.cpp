...
TablePage::UpdateStatus TablePage::UpdateTuple(Row &new_row, Row *old_row, Schema *schema, Txn *txn, LockManager *lock_manager,
  ...
  // If the slot number is invalid, abort.
  if (slot_num >= GetTupleCount()) {
    return UpdateStatus::slotNumInvalid;
  }
  uint32_t tuple_size = GetTupleSize(slot_num);
  // If the tuple is deleted, abort.
  if (IsDeleted(tuple_size)) {
    return UpdateStatus::tupleDeleted;
  }
  ...
  if (GetFreeSpaceRemaining() + tuple_size < serialized_size) {
    return UpdateStatus::notEnoughSpace;
  }
